...
import 'package:talabiapp/data/model/response/distance_model.dart';
import 'package:talabiapp/data/model/response/order_cancellation_body.dart';
import 'package:talabiapp/data/model/response/order_details_model.dart';
import 'package:talabiapp/data/model/response/order_model.dart';
import 'package:talabiapp/data/model/response/refund_model.dart';
import 'package:talabiapp/data/model/response/response_model.dart';
import 'package:talabiapp/data/model/response/store_model.dart';
import 'package:talabiapp/data/model/response/timeslote_model.dart';
...
import 'package:talabiapp/helper/network_info.dart';
...
class OrderController extends GetxController implements GetxService {
  ...
  bool isSwhing_Jawali=false;
   bool isSwhing_Jawalii=false;
  ...
  void changeIsShwoingJwali(bool state){
    isSwhing_Jawali=state;
    update();
  }

    void changeIsShwoingJwalii(bool state){
    isSwhing_Jawalii=state;
    update();
  }
  ...
  void pickRefundImage(bool isRemove) async {
    if(isRemove) {
      ...
      _refundImage = await ImagePicker().pickImage(source: ImageSource.gallery,imageQuality: 20);
      update();
    }
  }
  ...
  Future<void> placeOrder(PlaceOrderBody placeOrderBody, int? zoneID, Function(bool isSuccess, String? message, String orderID, int? zoneID, double amount, double? maximumCodOrderAmount) callback, double amount, double? maximumCodOrderAmount) async {
    ...
    _isLoading = false;
    print("this the data for adding to api ${response.statusCode} =   = =  = ");
    print("this the data for adding to api ${response.body} =   = =  = ");
    // print("this the data for adding to api ${response} =   = =  = ");
    if (response.statusCode == 200) {
  ...
  ParcelController parcelController =Get.find<ParcelController>();//addition
  final TextEditingController identifynumber=TextEditingController();
  
 bool loading = false ;
  loadingMethod(context)async{
   ...
   var data ;
                ...
                String token = await apiClient.token.toString();
                String url = "https://talabi-ye.com/api/v1/customer/order/flosk" ;
    ...
    Alert(
      ...
      desc: "أدخل رمزالتحقق الذي أرسل الى رقم حسابك",
      style:const AlertStyle(
        isCloseButton: false,
        descStyle: TextStyle(fontSize: 15,fontWeight: FontWeight.bold)),
        context: context,
        content: Column(
          children: [
            TextField(
           ...
           print(loading);
            ...
            child:const Text(
              ...
              style: TextStyle(color: Colors.white, fontSize: 20),
                  ...
                  Get.defaultDialog(title: "Warrning",middleText: "رقم الهاتف غيرمسجل لدى فلوسك");
                 }
